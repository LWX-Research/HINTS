...
class QuaternionBatchNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        orig_shape = x.shape
        x = x.reshape(x.shape[0], -1)  # [batch, dim*4]
        x = self.bn(x)
        return x.reshape(orig_shape)
...
def hamilton_to_quaternion(A: torch.Tensor) -> torch.Tensor:    
    ...
    y = -A[:, 1::2, 0::2] 
    z = A[:, 1::2, 1::2]
    q = torch.stack([w, x, y, z], dim=-1)  # [B, P, D, 4]
    return q / torch.norm(q, dim=-1, keepdim=True)

def qsvd_reconstruction(q: torch.Tensor, rank_q: int = 1) -> torch.Tensor:
    A = construct_hamilton_matrix(q)  # [B, 2P, 2D]    
    A_recon = qsvd_reconstruct(A, rank_q)
    q_recon = hamilton_to_quaternion(A_recon)  # [B, P, D, 4]
    return q_recon
...
def qsvd_main_left_singular(q: torch.Tensor) -> torch.Tensor:
    ...
    A = construct_hamilton_matrix(q)  # [B, 2P, 2D]
    U, S, Vh = torch.linalg.svd(A, full_matrices=False)
    assert U.shape == (B, 2*P, 2*P), f"Wrong U shape: {U.shape} vs {(B, 2*P, 2*P)}"
    u_vectors = U[..., :2]
